      ...
      <div className="mx-auto flex min-h-screen max-w-[1580px] flex-col p-4 lg:p-5">
        <header className="mb-4 flex flex-wrap items-center justify-between gap-4 rounded-[24px] border border-white/70 bg-white/82 px-5 py-3.5 shadow-[0_16px_40px_rgba(15,23,42,0.05)] backdrop-blur">
          <div>
            ...
            <h1 className="mt-1 text-[22px] font-semibold tracking-tight text-[#070261]">智能体工作台</h1>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button className="h-9 rounded-xl bg-[#161FAD] px-3.5 text-[13px] text-white shadow-[0_10px_24px_rgba(22,31,173,0.18)] hover:bg-[#1724D8]">
        ...
        <div className="grid flex-1 gap-4 xl:grid-cols-[260px_minmax(0,1fr)_340px]">
          <aside className="rounded-[24px] border border-white/70 bg-white/84 p-4 shadow-[0_16px_40px_rgba(15,23,42,0.045)] backdrop-blur">
            <div className="mb-4 flex items-center justify-between">
              ...
              <Button className="h-8.5 rounded-xl bg-[#161FAD] px-3 text-[13px] text-white hover:bg-[#1724D8]">
            ...
            <div className="space-y-2">
              {tasks.map((task) => (
                ...
                >
                  <p className={`truncate text-[13px] ${task.active ? "font-semibold text-[#070261]" : "font-medium text-slate-700"}`}>{task.title}</p>
          ...
          <section className="flex min-h-[720px] flex-col rounded-[24px] border border-white/70 bg-white/84 shadow-[0_16px_40px_rgba(15,23,42,0.045)] backdrop-blur">
            <div className="border-b border-slate-200/80 px-5 py-4">
              <p className="text-[11px] uppercase tracking-[0.18em] text-slate-400">Conversation</p>
              <h2 className="mt-1 text-[17px] font-semibold text-[#070261]">Agent 对话区域</h2>
            </div>

            <div className="flex-1 space-y-4 overflow-y-auto px-5 py-5">
              {messages.map((message, index) => (
                <div key={`${message.role}-${index}`} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[85%] rounded-[20px] px-4 py-3 text-[13px] leading-6 shadow-sm ${
                    message.role === "user"
            ...
            <div className="border-t border-slate-200/80 px-5 py-4">
              <div className="rounded-[20px] border border-slate-200 bg-slate-50/90 p-3">
                <textarea
                  className="min-h-[92px] w-full resize-none border-0 bg-transparent text-[13px] leading-6 outline-none placeholder:text-slate-400"
                  placeholder="继续向 Agent 提问，例如：请补充连接区长度与靶点共定位之间的交互分析。"
                />
                <div className="mt-3 flex items-center justify-between gap-3 border-t border-slate-200 pt-3">
                  <div className="flex items-center gap-2 text-slate-500">
                    <button className="rounded-xl p-2 transition hover:bg-white hover:text-[#161FAD]"><Paperclip className="h-4 w-4" /></button>
                    <button className="rounded-xl p-2 transition hover:bg-white hover:text-[#161FAD]"><Database className="h-4 w-4" /></button>
                  </div>
          ...
          <aside className="grid gap-4 lg:grid-rows-[minmax(0,1fr)_minmax(0,1fr)]">
            <section className="rounded-[24px] border border-white/70 bg-white/84 p-4 shadow-[0_16px_40px_rgba(15,23,42,0.045)] backdrop-blur">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.18em] text-slate-400">Plan</p>
                  <h2 className="mt-1 text-[17px] font-semibold text-[#070261]">计划</h2>
                </div>
                <div className="inline-flex items-center gap-1 rounded-full bg-[rgba(255,201,151,0.20)] px-2.5 py-1 text-[11px] font-medium text-[#8a5216]">
              ...
              <div className="space-y-3">
                {planSteps.map((step) => (
                  <article key={step.title} className="rounded-[18px] border border-slate-100 bg-slate-50/90 p-3.5">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <span className={`mt-2 inline-flex rounded-full border px-2.5 py-1 text-[10px] font-medium ${statusStyles[step.status]}`}>
            ...
            <section className="rounded-[24px] border border-white/70 bg-white/84 p-4 shadow-[0_16px_40px_rgba(15,23,42,0.045)] backdrop-blur">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.18em] text-slate-400">Result</p>
                  <h2 className="mt-1 text-[17px] font-semibold text-[#070261]">结果文件</h2>
                </div>
                <FolderOpen className="h-4 w-4 text-slate-400" />
              </div>

              <div className="mb-4 rounded-[18px] border border-slate-200 bg-slate-50/90 px-3 py-2">
              ...
              <div className="space-y-2.5">
                {files.map((file) => {
                  ...
                  return (
                    <button
                      key={file.name}
                      className="flex w-full items-start gap-3 rounded-[18px] border border-slate-100 bg-slate-50/90 px-3 py-3 text-left transition hover:border-[rgba(23,36,216,0.14)] hover:bg-white"
                    >
